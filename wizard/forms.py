...
class WizardStep(object):
    """
    a WizardStep object represents a single "step" or page for a Wizard

    This is really just a template of what a wizard step could look like, it does not need to be
    extended for any reason unless you want an empty post or prereq method.
    """
    def display(self):
        """
        this method will be called in order to reteieve all of the data, forms, formsets... needed
        to render the input page for a step in a form wizard
        """
        return {}

    def save(self):
        """
        this method will be called to actually process a submitted form for a step in a form wizard
        """
        pass

    def prereq(self):
        """
        this method will be called by the Wizard in order to determine that this step can be presented
        to the user to complete - or if the user must first complete a different step in the wizard

        ***if the user does indeed need to complete a prior step before this one, raise a PrereqMissing exception
        and set the step kwarg to which step the user must complete
        """
        pass

    def template(self):
        """
        this method does need to returna  Temp
        """
        return Template("")
